Feature: command-line arguments
  ...
  Rule: a tier's wall clock cannot fall below the duration of its longest scenario, so the slowest scenario in a tier is that tier's floor at any worker count. Measured on 2026-08-04, the help scenario above held that position. Its remaining assertions need real execution and stay at one process per command. Option coverage is a structural fact the parser answers from the same definition that builds both the help text and the manual, so it needs no process per command and it is asserted here instead of there.
