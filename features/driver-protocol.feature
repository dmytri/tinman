...
Feature: driver protocol
  ...
  Rule: a launch binds the system directories and whatever the session's sandbox spec names, and nothing more. Reaching a program outside that set is the `home: fixture` provision's job, per `scantlings/sandbox-spec.schema.json`. A program the sandbox cannot execute is a failed launch rather than a running session: answering with a session identifier for a process that never started leaves every later step asserting against a blank screen, and passing while it does so.
