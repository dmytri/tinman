...
- `@assistant` tier: the interactive-help scenarios. They share one expensive fixture and dominated the wall clock of the default tier, at 51 of its scenarios and roughly 38% of its time measured on 2026-08-05, while the assistant is not under active development. They run on request and at harbour. Strike the tag to bring them back when the assistant is worked on again.
- `@advisory` tier: queries a public advisory database over the network for the crates the released binary carries. It requires network egress and never sits on the inner loop, because it reddens on a publisher's action rather than on a change made here. Run it when deciding to ship, where a person is already weighing the answer.

## Recheck selection overrides the tier-sweep fallback

Project override, operator-authorized 2026-08-05. It narrows how a recheck set is derived and changes nothing about what custody guarantees: every scenario the change can reach is still run before a commit, and a role that cannot derive the set still sweeps.

The shared agreement says a touched verification-support hunk selects that tier's enumeration sweep, because support carries no planks. That fallback is coarse here for one structural reason: every step definition in the project lives in a single file, so touching three step definitions marks the whole file touched, which marks every tier reachable, which sweeps tiers the edit cannot reach. One custody pass swept all four tiers and roughly 253 seconds where one tier was owed.

The exact set is derivable, so derive it. A step definition carries its own pattern in its attribute, which is the same mapping a plank gives a production seam, and a shared helper has finite callers that terminate in step definitions. So the closure runs helper to callers to patterns to scenarios and always terminates.

Run the `select` command in `RIGGING.md` against the base commit. It reports the scenario set with its tier breakdown, and it reports what it could not resolve. **Run the selected set. Where `select` reports an unresolved symbol, sweep the tier that symbol serves, and only that tier.** A production seam with no plank is unresolved in exactly this sense and keeps its existing route.

The three shapes a role still runs whole are untouched: a tier-tag watch is one enumeration sweep by Captain's order, a harbour full regression runs every tier, and a red tier's dispatches stay focused runs.
...
`target/tinman-weather.jsonl` is yesterday's weather. Each tier enumeration sweep appends one line, and every `broad` command carries that append itself, so the record is produced by running the sweep and needs no runner support:
...
Methodology breaches surface as failing verification rather than as review comments. The rule set lives in `scantlings/verification-conformance` and is discharged by the `conformance` command, `ast-grep scan` over that rule set, configured by `sgconfig.yml`, which prints its status before propagating it. The duplication scanner is not a leg of this command. It reads `scantlings/duplication-allowance.json` for the size at or above which it is consulted, and reports zero groups at that size with no exemption available, because the scanner fingerprints shape with identifiers normalised and cannot tell a copied body from the shape small functions share. The size is what separates them, and the contract bounds it above, since a threshold raised far enough reads almost nothing and reports no group, which is indistinguishable from a clean tree. The same file carries the constant census allowance, which is joined by fingerprint in both directions and so needs a reader; both run from step definitions in `features/methodology-conformance.feature` rather than from a command leg. That keeps one rule in one checker. Which rules the set carries is not restated here, because this sentence enumerated six on the day the set held seven and nothing reddened: the floor asserts at least the rules it lists, so a sentence naming fewer decays in silence, which is the failure the durable-prose rules above describe. Read the set from the directories `sgconfig.yml` names. The floor is the step of that name in `features/methodology-conformance.feature:the verification-conformance rule set reports no match`, which joins the rule ids read from the directories `sgconfig.yml` names against the rules the step lists; read the floor from that step, which this sentence follows rather than fixes. The scenarios that run it are tagged `@conformance` in `features/methodology-conformance.feature`.
...
The duplication scanner reports a known weakness of its own: `cargo-dupes` reads functions and not items. Its unit census names closures, methods, functions and trait impl blocks, and it carries no unit kind for a constant at all, so no threshold or flag reaches one. The blindness was proven structural rather than a threshold effect on 2026-07-31 by a two-file probe: two byte-identical sources holding only constant declarations analysed as zero code units, so lowering `--min-nodes` and `--min-lines` to 1 changed nothing. Read a green from the scanner's own scenario as covering duplicated functions only.

That gap is carried rather than open. A second census reads the constants and is joined to the same allowance file in both directions by `features/methodology-conformance.feature:every duplicated constant the census reports is named as coincidence`, so a duplicated constant reddens where the scanner cannot see it. The two duplications that motivated adopting a scanner at all were collapsed rather than allowed: the copied status-line pair became one function, and the three deadline and key constants became one declaration and its imports. That is why the constant allowance is empty, and an empty allowance beside a passing census is the healthy resting state rather than an unread check, because the census is read in both directions and asserts what it read is not empty.
...
A coverage blind spot the summary does not announce: a scenario that drives `tinman` as a child process and then SIGKILLs it loses that child's coverage entirely, because a killed instrumented process never flushes its counters. The driver module read far below its real exercise for exactly that reason, while every scenario binding its planks passed.

The `killed-measured-child` rule in the rule set closed it by forbidding the outright kill in verification support, so a measured child is asked to stop and waited on instead. The figures moved with it: the driver module read 20.70% when the rule was written and 81.42% on `@sandbox` at the 2026-08-05 harbour. Read a coverage shortfall as unattributed, never as unreached, and judge reachability from the import and call graph, per the "Current design only" Article. Naming a shortfall as a dated figure rather than as a standing tier-by-tier claim is deliberate, because the number moves with the scenario mix and a claim of 0.00% everywhere decayed once already.

See `RIGGING.md` for the exact commands. Note the cucumber-rs constraint: `--name` and `--tags` are mutually exclusive, and the exclusion reaches the environment variable too, because `CUCUMBER_FILTER_TAGS` is that same `--tags` argument. A run passing both fails with `the argument '--name <regex>' cannot be used with '--tags <tagexpr>'`, whichever route the tag expression arrives by. So `focused` cannot carry the tag exclusion as a runner argument, by either route. It closes the hole a different way: the command reads the target scenario's own tag lines first and refuses to run a `@captain` or `@shipwright` target, so the exclusion the read contract asks for is enforced before the runner is invoked rather than inside it. A guard on the caller rather than the callee, because the callee cannot express it.
...
**An advisory on the crate graph reddens now; one on the npm graph is still found by asking.** When this decision was recorded, no command and no scenario ran an advisory check at all, and this advisory was found because somebody went looking. `features/methodology-conformance.feature:no advisory reaches a crate in the shipped graph` carries that question now, on the `@advisory` tier, for the crates the released binary ships.

The npm graph is the half still uncovered, and it was left that way on its own merits rather than by omission. It is dev rigging: its lockfile entries carry `"dev": true`, `npm/package.json` declares no dependencies, and the published npm artifact is a prebuilt binary beside `README.md` and `LICENSE`. So an advisory there reaches what builds this project and never what a user receives. Read a green default tier as saying nothing about advisories on either graph, because the tier that asks is opt-in and never sits on the inner loop.
...
The lever, when the tier does redden `features/methodology-conformance.feature:the most recent sweep of each tier is within its declared budget`, is the suite rather than the number. Two moves are available in order, and the first is already in hand. Measurement is the per-scenario duration record at `target/tinman-durations.jsonl`, taken by the before and after hooks in `tests/cucumber.rs` and read by `features/methodology-conformance.feature:the wake records how long each scenario took`. The second is fixture amortization, spent on the scenarios that record names as carrying the cost.
