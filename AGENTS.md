...
As of 2026-07-31 that move is started and not finished: the rule file is deleted, and the checker does not exist yet. `every test double in verification support carries its justification` is authored in `features/methodology-conformance.feature` and stands on the watchbill, and a focused run of it reports its first step matching no function. The residual weakness survives the move and is worth carrying with it: any check keyed on a name is blind to a double named outside its pattern. `LocalProvider` in verification support is the standing example, a real double correctly marked `@exceptional-double` whose name matches none of the four, and no type under `tests` matches them at all. Closing that needs a checker reading what a type does rather than what it is called.
